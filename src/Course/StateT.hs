...
state' ::
  (s -> (a, s))
  -> State' s a
state' = StateT . (P.fmap ExactlyOne)
...
previouslySeen' a = getT >>= (\s ->
                                if Set.member a s
                                  then pure True
                                  else const False <$> putT (Set.insert a s))
...
distinctF ::
  ...
  List a
  -> Optional (List a)
distinctF as = evalT (filtering (\a -> not <$> previouslySeenUnder100 a) as) Set.empty

previouslySeenUnder100 :: (Ord a, Num a) => a -> StateT (Set a) Optional Bool
previouslySeenUnder100 a = if a > 100
                             then StateT $ \_ -> Empty
                             else getT
                                  >>= (\s ->
                                      if Set.member a s
                                        then pure True
                                        else putT (Set.insert a s) >>= const (pure False))
...
instance Functor k => Functor (OptionalT k) where
  (<$>) ::
    ...
    -> OptionalT k a
    -> OptionalT k b
  (<$>) f (OptionalT kOa) = OptionalT $ (P.fmap f) <$> kOa
...
distinctG as = runOptionalT $ evalT (filtering filterFn as) Set.empty
  where filterFn ::
          (Integral a, Show a)
          => a
          -> StateT (Set a) (OptionalT (Logger Chars)) Bool
        filterFn a | a > 100 = do
                     liftLST $ log1 (listh "aborting > 100: " ++ show' a) Empty

                   | otherwise = do
                     s <- getT
                     let result = Set.notMember a s
                     putT $ Set.insert a s
                     if even a
                       then liftLST $ log1 (listh "even number: " ++ show' a)
                                           (pure result)
                       else pure result

liftLST :: Logger l (Optional a) -> StateT s (OptionalT (Logger l)) a
liftLST lgr = StateT $ \s -> OptionalT (P.fmap (,s) <$> lgr)
