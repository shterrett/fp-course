...
hex ::
  Parser Char
hex = thisMany 4 hexDigit
        >>= hexToChar
  where hexToChar hx =  case readHex hx of
                          (Full d) -> pure $ chr d
                          Empty -> unexpected hx
        hexDigit = satisfy isHexDigit
        unexpected s = P $ \_ -> UnexpectedString s
...
eof ::
  Parser ()
eof = P $ \input -> if input == ""
                      then Result "" ()
                      else ExpectedEof input
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma o c pa = between (charTok o) (charTok c)
                            $ sepby pa (charTok ',')
