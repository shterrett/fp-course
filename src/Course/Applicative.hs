...
filtering ::
  ...
  -> k (List a)
filtering f as =
    catOptional <$> (sequence $ map (test f) as)
  where test :: Applicative k => (a -> k Bool) -> a -> k (Optional a)
        test f' a = (\b -> if b then (Full a) else Empty) <$> f' a
        catOptional :: List (Optional a) -> List a
        catOptional Nil = Nil
        catOptional (Empty:.as') = catOptional as'
        catOptional (Full a :. as') = a:.(catOptional as')
