...
findLeft p (ListZipper ls a rs) =
    case break p ls of
      (_, Nil) -> isNotZ
      (lr, focus :. ll) -> isZ $ ListZipper ll focus (reverse lr ++  (a :. rs))
...
findRight p (ListZipper ls a rs) =
    case break p rs of
      (_, Nil) -> isNotZ
      (rl, focus :. rr) -> isZ $ ListZipper ((reverse rl) ++ a :. ls) focus rr
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop (ListZipper ls a (focus :. rs)) = ListZipper (a :. ls) focus rs
moveRightLoop (ListZipper ls a Nil) =
    case reverse ls of
      focus :. ls' -> ListZipper Nil focus (ls' ++ a :. Nil)
...
moveLeft (ListZipper (focus :. ls) a rs) = isZ $ ListZipper ls focus (a :. rs)
...
moveRight (ListZipper ls a (focus :. rs)) = isZ $ ListZipper (a :. ls) focus rs
...
swapLeft (ListZipper (a' :. ls) a rs) = isZ $ ListZipper (a :. ls) a' rs
...
swapRight (ListZipper ls a (a' :. rs)) = isZ $ ListZipper ls a' (a :. rs)
...
moveLeftN ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
moveLeftN i z | i == 0 = isZ z
              | i > 0 = (moveLeftN (i - 1)) -<< (moveLeft z)
              | i < 0 = moveRightN ((-1) * i) z
moveLeftN _ _ = error "unreachable"
...
moveRightN ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
moveRightN i z | i == 0 = isZ z
               | i > 0 = (moveRightN (i - 1)) -<< (moveRight z)
               | i < 0 = moveLeftN ((-1) * i) z
moveRightN _ _ = error "unreachable"
...
-- /Tip:/ Use `zipWith`
  (<*>) (ListZipper lsf af rsf) (ListZipper ls a rs) =
    ListZipper (zipWith ($) lsf ls) (af a) (zipWith ($) rsf rs)
...
instance Applicative MaybeListZipper where
  pure = isZ . pure
  (<*>) zfs zas = fromOptional $ lift2 (<*>) (toOptional zfs) (toOptional zas)
...
instance Traversable ListZipper where
  traverse f (ListZipper ls a rs) =
    ListZipper <$> (reverse <$> traverse f (reverse ls))
               <*> f a
               <*> (traverse f rs)
...
instance Traversable MaybeListZipper where
  traverse f z = case toOptional z of
                   Empty -> pure isNotZ
                   Full z' -> isZ <$> traverse f z'
